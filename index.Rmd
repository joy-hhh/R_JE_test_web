...
2. 프로젝트 폴더 경로에 전표 파일을 두고 읽는 명령으로 읽어와서 je라고 이름 붙인다.(이름 붙인 것을 객체라고 한다. je 객체) 윈도우즈 기준의 파일의 경우 파일명 뒤의 encoding 명시를 해주면 글자가 깨지지 않고 읽어온다. 윈도우 기준의 한글 형식에 맞출 수 있다. read_csv 명령 함수는 utf-8 encoding을 기본으로 하기 때문에 한글이 깨질 수 있다. 2기가 정도 되는 data file이 100초 정도 걸린다. (컴퓨터 성능에 따라서 다르다.) 몇십메가 정도의 data file은 몇초가 되기 전에 읽어버린다.[^2]   
3. 이름 붙인 je 객체를 Console에 나타내 확인한다. 
...
```{r, eval=FALSE, echo=TRUE}
B09_name <- je |> select(ACCTCD, ACCT_NM)  
B09_name <- B09_name[-which(duplicated(B09_name$ACCT_NM)),] 
B09 <- left_join(B09, B09_name, by = 'ACCTCD') 
```
  
1. 계정코드와 계정과목명 열을 선택한다.   
2. 중복되는 계정과목명을 삭제하고 하나씩만 남긴다.
3. 계정코드에 맞는 계정과목명을 B09 에 붙인다.  
...
반드시 한번 시도해 볼만 하다. 개발자가 되고자 하는 것이 아니다. 하늘을 나는 느낌과 컴퓨터를 말로 부려먹는 느낌을 가져보고 싶은 것이다.        
