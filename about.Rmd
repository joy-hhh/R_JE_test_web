...
[^2]: 원래 utf-8 encoding 형식으로 되어 있는 파일을 , locale=locale('ko',encoding='euc-kr)' 없이 읽으면 훨씬 더 빠르게 읽는다. 파일 형식에 대한 전환 과정이 필요 없기 때문이다. 대용량의 전표를 받아서 test 할 때는 utf-8 encoding 형식으로 요청하는 것이 좋은 방법이다.      
...
```{r}
## 상대계정분석에 있는 수)외환차익(채권)이 포함된 전표번호
test <- B09_all %>% 
  filter(ACCTCD == 56010601) %>% 
  select(JENO) %>% 
  distinct() %>% 
  pull()
...
je_tbl %>% 
  filter(JENO %in% test) %>% 
  select(JEDATE, JENO, amount, ACCT_NM, ACCTCD, record) -> test_tbl
